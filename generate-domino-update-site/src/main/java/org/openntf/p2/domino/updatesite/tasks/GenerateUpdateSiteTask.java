...
public class GenerateUpdateSiteTask implements Runnable {
	private static final Pattern FEATURE_FILENAME_PATTERN = Pattern.compile("^(.+)_([\\d\\.v\\-]+)\\.jar$"); //$NON-NLS-1$
	private static final ThreadLocal<DateFormat> TIMESTAMP_FORMAT = ThreadLocal.withInitial(() -> new SimpleDateFormat("yyyyMMdd"));
	...
	public void run() {
		...
		File notesJar = checkFile(new File(domino, "jvm" + File.separator + "lib" + File.separator + "ext" + File.separator + "Notes.jar"));
		...
		File eclipseLauncher = eclipsePlugins.listFiles(file -> file.getName().startsWith("org.eclipse.equinox.launcher_"))[0];
		...
		try {
			...
			new ProcessBuilder(
				...
				"-jar", eclipseLauncher.getAbsolutePath(),
				"-application", "org.eclipse.equinox.p2.publisher.EclipseGenerator",
				"-base", dest.getAbsolutePath(),
				"-source", dest.getAbsolutePath(),
				"-metadataRepository", destUri,
				"-artifactRepository", destUri,
				"-compress"
